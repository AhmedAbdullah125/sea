...
const VerfiyOtp = ({ phone }) => {
  ...
  async function onSubmit(values) {
    ...
    const res = await postToApi("/verify-otp", final);
    ...
    <Form {...form}>
      <form className="loginForm xl:space-y-8 space-y-6" onSubmit={form.handleSubmit(onSubmit)} >
        ...
        <div className="space-y-3">
          <Button
             className="h-12 px-6  bg-main-blue hover:bg-main-purple transtion-all duration-300 w-full text-xs font-bold   rounded-full flex items-center justify-between  hover:text-white  ">
            إرسال بواسطة واتساب
          ...
          <Button
             className="group h-12 px-6  bg-main-blue hover:bg-main-purple transtion-all duration-300 w-full text-xs font-bold   rounded-full flex items-center justify-between  hover:text-white  ">
