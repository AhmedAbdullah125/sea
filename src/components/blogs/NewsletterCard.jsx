...
const NewsletterCard = ({ bgColor, item, lang }) => {
  return (
    <motion.div
              ...
              <DialogDescription className="flex items-center justify-between">
                <div className="flex items-center gap-4 ">
                  ...
                  <div className="text-right">
                    <h4 className="text-xl font-bold">{lang === "ar" ? 'اذا حابب نرسلك كل ويكند ' : 'Subscribe to our weekly newsletter'}</h4>
                    <p className="text-xs text-main-blue">{lang === "ar" ? 'المقال الأكثر قراءةً خلال الأسبوع' : 'The most read article of the week'}</p>
                  </div>
                </div>
                <Link to={`/newsletter/${item?.slug}`} className=" h-12 px-8   bg-main-purple  text-xs font-bold  text-white hover:bg-main-blue hover:text-white transation-all duration-300  flex items-center justify-center gap-2 rounded-full ">
                  {lang === "ar" ? 'تـــابعنا الان' : 'Subscribe Now'}
