      ...
      <div className='container flex flex-col max-xl:gap-4 xl:flex-row items-center justify-between'>
        ...
        <div className='xl:w-1/3 w-full '>
          <div className=' text-center xl:text-start space-y-2 '>
              <h2 className='text-xs font-bold text-main-navy'>{data?.data?.data?.title }</h2>

            <h3 className='text-main-blue max-md:text-nowrap font-bold xl:text-5xl md:text-3xl text-xl xl:leading-relaxed'>
                {data?.data?.data?.description}
            </h3>
              <p className='text-main-navy text-xs'>{data?.data?.data?.mainDescription}</p>
          </div>
          <div className="xl:mt-32 mt-2 flex items-center max-xl:justify-center  gap-2 w-full">
            <button className="h-8  rounded-full   bg-main-purple text-white border-2 border-main-purple hover:bg-transparent hover:text-main-purple text-xs font-semibold px-6">إحجـــز رحلتك الان</button>
            <button className="group size-8 bg-main-purple border-2 border-main-purple hover:bg-transparent hover:text-main-purple rounded-full flex items-center justify-center">
              <svg
                className="text-white group-hover:text-main-purple "
                width="20"
                height="21"
                viewBox="0 0 20 21"
                fill="currentColor"
