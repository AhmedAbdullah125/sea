...
const fetchHotels = async (selectedCountry, selectedCity) => {
  const headers = {};
  if (localStorage.getItem("token")) headers.Authorization = `Bearer ${localStorage.getItem("token")}`;
  const response = await axios.get(
    `${API_BASE_URL}/filter-hotels?page=1&country_id=${selectedCountry||""}&city_id=${selectedCity||""}`,
    { headers }
  );
...
export const useGetHotels = (selectedCountry, selectedCity) => {
  ...
  const query = useQuery({
    ...
    queryFn: () => fetchHotels(selectedCountry, selectedCity),
